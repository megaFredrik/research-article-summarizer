...
if not {"title", "url"}.issubset(df.columns):
    st.error("The spreadsheet must contain title and url columns.")
    st.stop()
...
if st.button("Generate summaries", type="primary", disabled=not bool(os.getenv("OPENAI_API_KEY"))):
    ...
    for i, row in df.iterrows():
        try:
            text = fetch_article(str(row["url"]))
            summary = summarize(str(row["title"]), str(row["url"]), text, str(row.get("notes", "")))
            results.append({"title": row["title"], "summary": summary, "status": "OK"})
        except Exception as e:
            results.append({"title": row["title"], "summary": f"ERROR: {e}", "status": "ERROR"})
...
if "results" in st.session_state:
    ...
    md = to_markdown(st.session_state.results)
    st.download_button("Download Markdown", md, "research-summaries.md", "text/markdown")
    html = "<html><body><h1>Research Reading Summaries</h1>" + "".join(f"<article><pre style='white-space:pre-wrap'>{r['summary']}</pre></article><hr>" for r in st.session_state.results) + "</body></html>"
    st.download_button("Download shareable HTML", html, "research-summaries.html", "text/html")

st.divider()
st.caption("Privacy: article text is sent to the configured AI provider only during summarization. Avoid sensitive content unless approved for that provider/account.")
